...
// setupConfig reads the configuration and prepares the agent's config.
// It's called before running the command's RunE function.
func (c *cli) setupConfig (cmd *cobra.Command, args []string) error {
	...
	c.cfg.StartJoinAddrs = viper.GetStringSlice("start-join-addr")
...
// run runs our executables logic by creating an agent, handle signals from the
// OS and gracefully shuts down the agent when the os terminates the program.
