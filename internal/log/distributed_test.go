...
func TestMultipleNodes(t *testing.T) {
	...
	for i := 0; i < nodeCount; i++ {
		...
		// shorten the default Raft timeout configs
		config := log.Config{}
		...
		// the first server bootstraps the cluster & becomes leader before joining the other two.
		if i == 0 {
	...
	// we testing by appending some records to our leader server, checking that
	// raft has replicated the records to its followers.
	records := []*api.Record{
		{Value: []byte("first")},
		{Value: []byte("second")},
	}
	...
	// Check that the leader stop replicating to a server that's left the cluster
	err := logs[0].Leave("1")
	require.NoError(t, err)
